...
class gui(object):
	...
	def setup_buttons(self, user_in):
		...
		for index in range(2):
			enable_button = tkinter.Button(self.root, \
					...
					command=(lambda: self.enable_pi_bot(), \
					lambda: self.disable_pi_bot())[index])
		...
		self.joystick_reset_button = tkinter.Button( \
				textvariable=user_in.reset_button_text, \
				command=lambda : user_in.reset_user_input(self.root))
		self.joystick_reset_button.pack()
	def __repr__(self):
		pass
	def update_gui(self, user_in):
		...
		if user_in.check_user_input == 1:
			user_in.check_joysticks()
			for joystick_i in range(pygame.joystick.get_count()):
				active_joy = user_in.joystick_active(joystick_i)
	...
	def select_enable_mode(self, enable_mode):
		...
		TELEOP, AUTO, TEST
		'''
		if self.selected_enable_mode != enable_mode and \
				pi_bot_info.enable_state == "DISABLED":
			self.selected_enable_mode = enable_mode
	def enable_pi_bot(self):
		'''Tells comms_out to enable the robot in the selected mode'''
		if pi_bot_info.enable_state == pi_bot_info.state_disabled:
			comms_out.send_string("ENABLE_" + \
					self.__enable_mode_options[self.selected_enable_mode])
		#temp set info
		pi_bot_info.enable_state = pi_bot_info.state_teleop_enabled
	def disable_pi_bot(self):
		'''Tells comms_out to disable the robot regardless of selected mode'''
		if pi_bot_info.enable_state != pi_bot_info.state_disabled:
			comms_out.send_string("DISABLE")
		#temp set info
		pi_bot_info.enable_state = pi_bot_info.state_disabled
